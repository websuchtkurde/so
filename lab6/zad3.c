...
// Funkcja czytelnika
void czytelnik(int id, int sem_id, char *shared_memory) {
    sem_operation(sem_id, 0, -1); // Blokowanie pisarzy
    sem_operation(sem_id, 1, 1);  // Zgłaszanie wejścia do sekcji krytycznej dla czytelników

    // Odczytanie wiadomości
    char message[SHARED_MEMORY_SIZE];
    strncpy(message, shared_memory, SHARED_MEMORY_SIZE);

    sem_operation(sem_id, 1, -1); // Zwolnienie sekcji krytycznej dla czytelników
    sem_operation(sem_id, 0, 1);  // Zwolnienie pisarzy
...
// Funkcja pisarza
void pisarz(int id, int sem_id, char *shared_memory) {
    sem_operation(sem_id, 0, -1); // Blokowanie czytelników
    sem_operation(sem_id, 2, 1);  // Zgłaszanie wejścia do sekcji krytycznej dla pisarzy
    ...
    sem_operation(sem_id, 2, -1); // Zwolnienie sekcji krytycznej dla pisarzy
    sem_operation(sem_id, 0, 1);  // Zwolnienie czytelników
}
...
int main() {
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (child_pid == -1) {
            ...
            if (i % 2 == 0) {
                // Czytelnik
                czytelnik(i, sem_id, shared_memory);
            } else {
                // Pisarz
                pisarz(i, sem_id, shared_memory);
            }
            exit(0);
        }
    }
